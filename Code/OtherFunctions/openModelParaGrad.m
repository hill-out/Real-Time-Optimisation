function [dfun] = openModelParaGrad(u,dPara)
...
for i = 1:3
    % dCdT
    newC = openModelPara(u,dPara,baseC,dT(i,:));
...
for j = 1:numel(u)
    % dCdu
    newC = openModelPara(u+dU(j,:),dPara,baseC);
...
for k = 1:numel(u)
    newCk = openModelPara(u+dU(k,:),dPara,baseC);
    newPhik = phiFun(u+dU(k,:),newCk);
    newg1k = g1Fun(u+dU(k,:),newCk);
    newg2k = g2Fun(u+dU(k,:),newCk);
    
    for j = 1:numel(u)
        newCj = openModelPara(u+dU(k,:)+dU(j,:),dPara,baseC);
    ...
    for i = 1:3
        newCi = openModelPara(u+dU(k,:),dPara,baseC,dT(i,:));

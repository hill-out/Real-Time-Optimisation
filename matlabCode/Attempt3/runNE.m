...
mCostGrad0 = gradFD(@(x)(model.convCost(x)), 1,  u0, delu);
mConsGrad0 = gradFD(@(x)(model.convCons(x)), 2,  u0, delu);
...
while unsolved
    ...
    function [costGrad, consGrad] = gradNE
        ...
        du = repmat(u0, 2, 1) + 0.01*eye(2); %modified values of u
        ...
        dcostdu0 = (costdu0 - repmat(cost0,2,1))./0.01;
        dconsdu0 = (consdu0 - repmat(permute(cons0,[3,2,1]),2,1))./0.01;
        dHdu = (Cdu - repmat(reshape(C0,1,1,4),2,1))./0.01;
        
        % first order gradients of steady state model (dp)
        p = [model.k(1), model.k(2), model.c_in(1)];
        dp = repmat(p, 3, 1) + 0.01*eye(3);
        Cdp = zeros(1,3,4);
        ...
        dHdp = (Cdp - repmat(reshape(C0,1,1,4),1,3))/0.01;
        ...
        for dp_i = 1:3
            ...
            dcostdudp(:,dp_i) = (costdu - repmat(cost0,2,1))/0.01;
        end
        
        ddcostdpdu = (dcostdudp - repmat(dcostdu0,1,3))./0.01;
        
        % second order gradients for cost (dudu)
        eye3 = repmat(eye(2),1,1,2);
        dudu = repmat(u0, 2, 1, 2) + 0.01*eye3 + 0.01*permute(eye3,[3,2,1]); %modified values of u
        ...
        for du_i = 1:2
            ...
            dcostdudu(:,du_i) = (costdudu - repmat(cost0,2,1))/0.01;
        end
        
        ddcostdudu = (dcostdudu - repmat(dcostdu0,1,2))./0.01;
        ...
        for dp_i = 1:3
            ...
            dconsdudp(:,dp_i,:) = (consdu - repmat(permute(cons0,[3,2,1]),2,1))/0.01;
        end
        
        ddconsdpdu = (dconsdudp - repmat(dconsdu0,1,3))./0.01;
        ...
        for du_i = 1:2
            ...
            dconsdudu(:,du_i,:) = (consdudu - repmat(permute(cons0,[3,2,1]),2,1))/0.01;
        end
        
        ddconsdudu = (dconsdudu - repmat(dconsdu0,1,2))./0.01;
        
        gammaCost = mCostGrad0 + ddcostdudp*

function [u_c_x_opt, f, eflag, outpt, F_P] = RunOptimization_M_modified_UU(u_x_0, upk_s, ck, F_P_demand, Theta, A_us, b_u, Modified_Opt_parameters, opts, Constrain)
...
[u_c_x_opt, f, eflag, outpt] = fmincon(@(u_x)objfun(u_x, F_P_demand),...
    ...
    function y = objfun(u_x, F_P_demand)
        if ~isequal(u_x,Last_u_x) % Check if computation is necessary
            ...
            Last_u_x  = u_x;
            F_P_constraint = g + epsilon_g_k + Lambda_g_k'*inv_A_us*(u_s-upk_s);
            
            Last_f   = phi_u + phi_c + epsilon_phi_k + lambda_phi_k'*inv_A_us*(u_s-upk_s) ;
            Last_c   = [];
            if Constrain.FP == 1
